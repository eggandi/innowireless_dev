...
struct CleanupHead g_cleanup_head = TAILQ_HEAD_INITIALIZER(g_cleanup_head);

static void enqueue_for_cleanup(struct J29451GNSSDataBufEntry *e)
{
  // 이 시점에 e->entries 리스트 포인터가 꼬이지 않도록 주의
  TAILQ_INSERT_TAIL(&g_cleanup_head, e, entries);
}
/**
  ...
  if (entry) {
    ...
    if (buf->entry_num == kJ29451GNSSDataBufEntryNum_Max) {
      ...
      TAILQ_REMOVE(&(buf->head), first_entry, entries);
			pthread_mutex_unlock(&g_j29451_mib.mtx);
      buf->entry_num--;
      enqueue_for_cleanup(first_entry);
    }
...
/* 20250527 DEBUGING 
 * 엔트리 제거하기 위해서 TAILQ_FOREACH_SAFE()를 사용하면서 오류 발생
 * TAILQ_FOREACH_SAFE()를 사용하지 않고, TAILQ_PREV()를 사용하여 엔트리를 제거하는 방식으로 변경
 */
...
static void j29451_RemoveAllPrevGNSSDataBufEntry(struct J29451GNSSDataBufEntry *entry)
{
  ...
  struct J29451GNSSDataBufEntry *tmp1, *tmp2;
	// 기준 엔트리와 비교를 위해서 버퍼 내 첫번째 엔트리(=가장 오래된 엔트리)를 가져온다.
	tmp2 = TAILQ_FIRST(&(buf->head));
	if(!tmp2) {
		return;
	}else{
		// 기준 엔트리와 첫번째 엔트리가 같으면 삭제 할지 않고 반환한다.
		if(tmp2 == entry) {
			return;
		}
	}
	do{
		// 기준 엔트리의 이전 엔트리가 존재하면 엔트리에서 제외하고 다시 이전 엔트리를 탐색
		if(buf->entry_num == 1)
		{
			return;
		}
		tmp1 = TAILQ_PREV(entry, J29451GNSSDataBufEntryHead, entries);
		if(tmp1) {
			if (tmp1 == tmp2) {
				// 루프 방지를 위해 첫번째 엔트리까지만 이전 엔트리를 탐색해 제거
...
static struct J29451GNSSDataBufEntry * j29451_GetNextGNSSDataBufEntryInSafeMode(void)
{
   ...
   */
    ...
    if (entry) {
      ...
      // 202050527 DEBUGING ** invalid used error(free 된 entry를 사용)
