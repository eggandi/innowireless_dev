...
bool INTERNAL j29451_CheckGNSSDataBufEntryEpochTimeIncrease(struct J29451GNSSDataBufEntry *entry)
{
  ...
  printf("buf->entry_num=%d\n", buf->entry_num);
	if (buf->entry_num < 2) {
		// 엔트리가 없으면 epoch time 증가 여부를 확인할 수 없다.
		return true;
	}
	struct J29451GNSSDataBufEntry *prev = TAILQ_PREV(entry, J29451GNSSDataBufEntryHead, entries);
	struct J29451GNSSDataBufEntry *first = TAILQ_FIRST(&(buf->head));
	for(J29451GNSSDataBufEntryNum count = 0; (count < buf->entry_num) && (prev == NULL); count++) {
		printf("[%d] prev is NULL, j29451_CheckGNSSDataBufEntryEpochTimeIncrease: first=%p, entry=%p, prev=%p\n",	count, first, entry, prev);
		struct J29451GNSSDataBufEntry *next = TAILQ_NEXT(first, entries);
		printf("[%d] prev is NULL, j29451_CheckGNSSDataBufEntryEpochTimeIncrease: first=%p, next=%p, entry=%p, prev=%p\n",	count, first, next, entry, prev);
		if(next != NULL) {
			first = next;
		}
	}
  if (prev) {
		// 20250527 DEBUGING: free가 된 엔트리를 읽음
...
void INTERNAL * j29451_GNSSDataUpdateThread(void *arg)
{
  ...
  while (gps_waiting(gps_data, GNSS_DATA_WAITING_USEC)) {
    ...
    j29451_UpdateGNSSDataBufEntry(gps_data);
		...
		// 20250527 DEBUGING 성동규 수정 
		/* GNSS 데이터 Queue에서 제거된 엔트리 모아서 free */
