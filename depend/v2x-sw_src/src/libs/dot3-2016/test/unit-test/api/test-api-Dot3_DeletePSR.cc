/**
 * @file
 * @brief Dot3_DeletePSR() API에 대한 단위테스트 구현 파일
 * @date 2020-07-18
 * @author gyun
 */
...
/**
 * @brief Dot3_DeletePSR() API 호출 시 해당 PSR이 정상적으로 삭제되는 것을 확인한다.
 */
TEST(Dot3_DeletePSR, NORMAL)
{
  InitTestEnv();
  ...
  for (unsigned int i = 0; i < kDot3PSRNum_Max; i++) {
    ...
    ASSERT_EQ(Dot3_GetPSRWithPSID(psid + i, &psr_r), kDot3Result_Success);
  ...
  /*
   * 각 PSR이 정상적으로 삭제되는 것을 확인한다.
   */
  psid = 0;
  for (unsigned int i = 0; i < kDot3PSRNum_Max; i++) {
    ASSERT_EQ(Dot3_DeletePSR(psid + i), (int)--psr_num);
    ASSERT_EQ(Dot3_GetPSRNum(), psr_num);
    ASSERT_EQ(Dot3_GetPSRWithPSID(psid + i, &psr_r), -kDot3Result_NoSuchPSR);
  }
  ASSERT_EQ(Dot3_GetPSRNum(), 0UL);
...
/**
 * @brief Dot3_DeletePSR() API 호출 시 전달되는 PSID 파라미터 값에 따른 동작을 확인한다.
 */
TEST(Dot3_DeletePSR, CHECK_PARAM_PSID)
{
  InitTestEnv();
   ...
   */
  ASSERT_EQ(Dot3_DeletePSR(kDot3PSID_Max + 1), -kDot3Result_InvalidPSID);
  ASSERT_EQ(Dot3_GetPSRNum(), psr_num);

  /*
   * 등록되어 있지 않은 PSID 전달 시 실패하는 것을 확인한다.
   */
  ASSERT_EQ(Dot3_DeletePSR(psid + 1), -kDot3Result_NoSuchPSR);
  ASSERT_EQ(Dot3_GetPSRNum(), psr_num);

  /*
   * 등록된 PSID 전달 시 정상적으로 삭제되는 것을 확인한다.
   */
  ASSERT_EQ(Dot3_DeletePSR(psid), 0);
  ASSERT_EQ(Dot3_GetPSRNum(), 0UL);
