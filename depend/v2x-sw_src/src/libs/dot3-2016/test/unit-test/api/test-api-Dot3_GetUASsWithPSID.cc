/** 
 * @file
 * @brief Dot3_GetUASsWithPSID() API에 대한 단위테스트 구현 파일
 * @date 2020-07-29
 * @author gyun
 */
...
/**
 * @brief Dot3_GetUASsWithPSID() API 호출 시 해당되는 UAS들이 정상적으로 반한되는 것을 확인한다.
 */
TEST(Dot3_GetUASsWithPSID, NORMAL)
{
  InitTestEnv();
...
#define WSA_SENDER_NUM (2)
   ...
   */
  set = Dot3_GetUASsWithPSID(psid[0], &ret);
...
/**
 * @brief Dot3_GetUASsWithPSID() API 호출 시 UAS 테이블에 UAS가 하나도 없을 때의 동작을 확인한다.
 */
TEST(Dot3_GetUASsWithPSID, NO_UAS)
{
  InitTestEnv();
...
/**
 * @brief Dot3_GetUASsWithPSID() API 호출 시 전달되는 PSID 파라미터 값에 따른 동작을 확인한다.
 */
TEST(Dot3_GetUASsWithPSID, CHECK_PARAM_PSID)
{
  InitTestEnv();
...
#define WSA_SENDER_NUM (2)
  ...
  /*
   * API 호출 시, 등록된 PSID를 전달하면 UAS가 정상 반환되는 것을 확인한다.
   *  - 두개의 UAS가 반환된다. (두 송신자가 송신한 각 정보)
   */
  set = Dot3_GetUASsWithPSID(psid[0], &ret);
  ASSERT_TRUE(set != NULL);
  ASSERT_EQ(set->num, 2UL);
   ...
   */
  set = Dot3_GetUASsWithPSID(psid[0] + 1, &ret);
  ASSERT_TRUE(set != NULL);
  ASSERT_EQ(set->num, 0UL);
  free(set);

  /*
   * API 호출 시, 유효하지 않은 PSID를 전달하면 0개의 UAS가 반환되는 것을 확인한다.
   */
  set = Dot3_GetUASsWithPSID(kDot3PSID_Max + 1, &ret);
  ASSERT_TRUE(set != NULL);
  ASSERT_EQ(set->num, 0UL);
  free(set);
...
/**
 * @brief Dot3_GetUASsWithPSID() API 호출 시 전달되는 NULL 파라미터에 따른 동작을 확인한다.
 */
TEST(Dot3_GetUASsWithPSID, CHECK_PARAM_NULL)
{
  InitTestEnv();
...
#define WSA_SENDER_NUM (2)
  ...
  /*
   * API 호출 시, ret 파라미터를 NULL로 전달하면 실패하는 것을 확인한다.
   */
  set = Dot3_GetUASsWithPSID(psid[0], NULL);
  ASSERT_TRUE(set == NULL);
  free(set);
