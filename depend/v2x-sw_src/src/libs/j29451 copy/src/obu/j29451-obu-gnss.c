...
static int j29451_GetGNSSDataFromGPSD(struct J29451GNSSData *gnss)
{
  ...
  struct J29451GNSSDataBufEntry *entry = j29451_GetGNSSDataBufEntryToProcess();
	printf("j29451_GetGNSSDataFromGPSD: entry=%p\n", entry);
  if (!entry) {
    Err("Fail to get GNSS data buf entry\n");
    return -kJ29451Result_InsufficientData;
  }
	printf("j29451_GetGNSSDataFromGPSD: entry->ref_count=%d\n", entry->ref_count);
  memcpy(gnss, &(entry->gnss), sizeof(struct J29451GNSSData));
	...
	/*
	 * GNSS 데이터 버퍼 엔트리의 epoch time이 적절한지 확인한다.
	 */
	if (j29451_CheckGNSSDataBufEntryEpochTimeIncrease(entry) == false) {
		Err("GNSS data buf entry epoch time increase is invalid\n");
		return -kJ29451Result_InsufficientData;
	}
#endif // _UNIT_TEST_
