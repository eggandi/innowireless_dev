...
/**
 * @brief RA에게 익명인증서 발급을 요청한다.
 * @param[in] params 익명인증서 발급요청 파라미터
 * @param[in] res 익명인증서 발급요청 결과가 저장될 구조체 포인터
 *
 * 익명인증서발급요청서를 생성하여 RA에 전송하고 응답문을 수신하여 결과를 반환한다.
 */
void INTERNAL dot2_RequestPseudonymCertProvisioning(
  ...
  struct Dot2PseudonymIdCertProvisioningRequestResult *res)
{
  ...
  if ((cr_info.https.pcp_url == NULL) ||
      (cr_info.https.rca_tls_cert_file_path == NULL)) {
    Err("Fail to request pseudonym cert provisioning - no https connection info\n");
    ret = -kDot2Result_LCM_HTTPS_NoConnectionInfo;
    goto err;
  }

  /*
   * 서명용 임시 개인키/공개키, 확장함수키를 생성한다.
   */
  ...
  memcpy(&verify_key, &(g_dot2_mib.lcm.test.pseudonym_cert.tv.verify_key), sizeof(struct Dot2ECKeyPairOcts));
  memcpy(&cert_enc_key, &(g_dot2_mib.lcm.test.pseudonym_cert.tv.encryption_key), sizeof(struct Dot2ECKeyPairOcts));
  memcpy(&verify_exp_key, &(g_dot2_mib.lcm.test.pseudonym_cert.tv.verify_exp_key), sizeof(struct Dot2AESKey));
  memcpy(&cert_enc_exp_key, &(g_dot2_mib.lcm.test.pseudonym_cert.tv.encryption_exp_key), sizeof(struct Dot2AESKey));
#endif
   ...
   */
#if defined(_FFASN1C_)
  req = dot2_ffasn1c_ConstructSecuredCertProvisioningRequest(kDot2CMHType_Pseudonym,
                                                             current_time,
                                                             start_time,
                                                             &(verify_key.pub_key),
...
#elif defined(_OBJASN1C_)
  req = dot2_objasn1c_ConstructSecuredCertProvisioningRequest(kDot2CMHType_Pseudonym,
                                                              current_time,
                                                              start_time,
                                                              &(verify_key.pub_key),
                                                              &(cert_enc_key.pub_key),
...
#error "3rd party asn.1 library is not defined"
  ...
  memcpy(req_h8, g_dot2_mib.lcm.test.pseudonym_cert.tv.prov_req_h8, 8);
#endif

  /*
   * RA에게 익명인증서발급요청문을 전송하고, 익명인증서발급응답문을 수신한다.
   */
  ret = dot2_HTTPS_POST(cr_info.https.pcp_url, cr_info.https.rca_tls_cert_file_path, req, req_size, &ack);
  if (ret < 0) {
    goto err;
  }

  /*
   * 수신된 익명인증서발급응답문을 처리한다.
   */
  Dot2Time32 cert_dl_time;
  char *cert_dl_url;
  Dot2CMHType cert_type = kDot2CMHType_Pseudonym;
  ...
  Log(kDot2LogLevel_Event, "Success to request pseudonym cert provisioning\n");
  return;
