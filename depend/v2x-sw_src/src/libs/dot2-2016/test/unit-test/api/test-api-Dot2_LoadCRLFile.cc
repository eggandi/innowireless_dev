/**
  * @file
  * @brief Dot2_LoadCRLFile() API 단위테스트
  * @date 2023-03-03
  * @author gyun
  */
 ...
 */
TEST(Dot2_LoadCRLFile, HASH_CRL)
{
  ASSERT_EQ(Dot2_Init(kDot2LogLevel_Err, kDot2SigningParamsPrecomputeInterval_Default, "/dev/urandom", kDot2LeapSeconds_Default), kDot2Result_Success);

  Dot2Cert ec{}, eca{}, ra{}, rca{}, crlg{}, ica{}, pca{};
  const char *crl_file = "test/test-file/lcm/bluetech/crl/hash_crl.oer";
   ...
   */
  {
    ...
    // 정상동작 확인
    ASSERT_EQ(Dot2_LoadCRLFile(crl_file), kDot2Result_Success);
 ...
 */
TEST(Dot2_LoadCRLFile, LV_CRL)
{
  ASSERT_EQ(Dot2_Init(kDot2LogLevel_Err, kDot2SigningParamsPrecomputeInterval_Default, "/dev/urandom", kDot2LeapSeconds_Default), kDot2Result_Success);

  Dot2Cert ec{}, eca{}, ra{}, rca{}, crlg{}, ica{}, pca{};
  const char *crl_file = "test/test-file/lcm/bluetech/crl/lv_crl.oer";
  uint8_t pc_h8[8];
 ...
 * @brief 유효하지 않은 파라미터
 */
TEST(Dot2_LoadCRLFile, INVALID_PARAMS)
{
  ASSERT_EQ(Dot2_Init(kDot2LogLevel_Err, kDot2SigningParamsPrecomputeInterval_Default, "/dev/urandom", kDot2LeapSeconds_Default), kDot2Result_Success);

  Dot2Cert ec{}, eca{}, ra{}, rca{}, crlg{}, ica{}, pca{};
  const char *crl_file = "test/test-file/lcm/bluetech/crl/lv_crl.oer";
  const char *invalid_crl_file = "aaa.oer";
  const char *empty_crl_file = "test/test-file/lcm/bluetech/crl/empty_crl.oer";
  uint8_t ac_h10[10];
  uint8_t ac_h1;
  struct Dot2HashBasedCertRevocationEntry *app_cert_revo_entry = nullptr;
  ...
  {
    // 널 파라미터 전달
    ASSERT_EQ(Dot2_LoadCRLFile(nullptr), -kDot2Result_NullParameters);

    // 유효하지 않은 파일경로 전달
    ASSERT_EQ(Dot2_LoadCRLFile(invalid_crl_file), -kDot2Result_FILE_Access);

    // 유효하지 않은 길이를 갖는 CRL 파일 전달
    ASSERT_EQ(Dot2_LoadCRLFile(empty_crl_file), -kDot2Result_FILE_InvalidLength);
  }
 ...
 * @brief CRLG 인증서 없음
 */
TEST(Dot2_LoadCRLFile, NO_CRLG_CERT)
{
  ASSERT_EQ(Dot2_Init(kDot2LogLevel_Err, kDot2SigningParamsPrecomputeInterval_Default, "/dev/urandom", kDot2LeapSeconds_Default), kDot2Result_Success);
   ...
   */
  {
    ...
    // CRLG 인증서 등록 없이 호출
    ASSERT_EQ(Dot2_LoadCRLFile(crl_file), -kDot2Result_NoSignerIdCertInTable);
  }
 ...
 */
TEST(Dot2_LoadCRLFile, INVALID_DATE)
{
  ASSERT_EQ(Dot2_Init(kDot2LogLevel_Err, kDot2SigningParamsPrecomputeInterval_Default, "/dev/urandom", kDot2LeapSeconds_Default), kDot2Result_Success);
   ...
   */
  {
    ...
    ASSERT_EQ(Dot2_LoadCRLFile(crl_file), -kDot2Result_CRL_InvalidPeriod);
  }
