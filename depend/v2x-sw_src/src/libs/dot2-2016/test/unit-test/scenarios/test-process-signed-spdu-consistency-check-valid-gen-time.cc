/** 
  * @file 
  * @brief 서명 SPDU의 생성시각과 인증서유효기간 사이의 consistency 체크 기능 단위테스트
  * @date 2022-01-06 
  * @author gyun 
  */
...
/**
 * @brief 서명 SPDU의 생성시각과 인증서유효기간 사이의 consistency 체크 동작을 확인한다.
 */
TEST(PROCESS_SIGNED_SPDU_CONSISTENCY_CHECK_SPDU_GEN_TIME_IN_CERT_VALID_TIME, CHECK_SPDU_GEN_TIME)
{
  ...
  */
  ...
  /*
   * [TEST] SPDU 내 생성시각이 존재하지 않으면, "성공"이 반환되는 것을 확인한다.
   */
  {
  ...
  /*
   * [TEST] SPDU 내 생성시각이 인증서 유효기간 내에 있으면, "성공"이 반환되는 것을 확인한다.
   */
  {
    struct V2XPacketParseData *parsed = V2X_AllocateDSRCPacketParseData(nullptr, 0, nullptr);
    ASSERT_TRUE(parsed != nullptr);
    ...
    ASSERT_FALSE(g_callbacks.entry[spdu_cnt-1].parsed->spdu.signed_data.ext_h_present); // ext hash 불포함
    ASSERT_EQ(g_callbacks.entry[spdu_cnt-1].parsed->spdu.signed_data.psid, 135U); // PSID=135
    ASSERT_TRUE(g_callbacks.entry[spdu_cnt-1].parsed->spdu.signed_data.gen_time_present); // 생성시각 포함
    ASSERT_FALSE(g_callbacks.entry[spdu_cnt-1].parsed->spdu.signed_data.expiry_time_present); // 만기시각 불포함
    ASSERT_FALSE(g_callbacks.entry[spdu_cnt-1].parsed->spdu.signed_data.gen_location_present); // 생성좌표 불포함
    ASSERT_TRUE(g_callbacks.entry[spdu_cnt-1].parsed->ssdu != nullptr); // 페이로드 비교
    ASSERT_EQ(g_callbacks.entry[spdu_cnt-1].parsed->ssdu_size, payload_size); // 페이로드 비교
  ...
  /*
   * [TEST] SPDU 내 생성시각이 인증서 유효기간 시작시점보다 과거이면, "실패"가 반환되는 것을 확인한다.
   */
  {
    struct V2XPacketParseData *parsed = V2X_AllocateDSRCPacketParseData(nullptr, 0, nullptr);
    ASSERT_TRUE(parsed != nullptr);
    uint8_t *spdu = g_sample_spdu_consistency_check_gentime_is_earlier_than_cert_valid_start;
    size_t spdu_size = g_sample_spdu_consistency_check_gentime_is_earlier_than_cert_valid_start_size;
    struct Dot2SPDUProcessParams params = {0, 135, 374063230L, 1271023340L};
    ASSERT_EQ(Dot2_ProcessSPDU(spdu, spdu_size, &params, parsed), kDot2Result_Success);
    ...
    WAIT_MSG_PROCESS_CALLBACK;
    ASSERT_EQ(g_callbacks.cnt, ++spdu_cnt);
    ASSERT_EQ(g_callbacks.entry[spdu_cnt-1].result, -kDot2Result_SPDUConsistency_GenTimeIsNotInSignerValidPeriod);
  }

  /*
   * [TEST] SPDU 내 생성시각이 인증서 유효기간 종료시점보다 미래이면, "실패"가 반환되는 것을 확인한다.
   */
  {
    struct V2XPacketParseData *parsed = V2X_AllocateDSRCPacketParseData(nullptr, 0, nullptr);
    ASSERT_TRUE(parsed != nullptr);
    uint8_t *spdu = g_sample_spdu_consistency_check_gentime_is_later_than_cert_valid_end;
    size_t spdu_size = g_sample_spdu_consistency_check_gentime_is_later_than_cert_valid_end_size;
    struct Dot2SPDUProcessParams params = {0, 135, 374063230L, 1271023340L};
    ASSERT_EQ(Dot2_ProcessSPDU(spdu, spdu_size, &params, parsed), kDot2Result_Success);
 ...
 */
TEST(PROCESS_SIGNED_SPDU_CONSISTENCY_CHECK_SPDU_GEN_TIME_IN_CERT_VALID_TIME, NOT_VERIFY)
{
  ASSERT_EQ(Dot2_Init(kDot2LogLevel_Err, kDot2SigningParamsPrecomputeInterval_Default, "/dev/urandom", kDot2LeapSeconds_Default), kDot2Result_Success);
  Dot2_RegisterProcessSPDUCallback(Dot2Test_ProcessSPDUCallback);
  Dot2Test_InitProcessSPDUCallbackList();
  ...
  */
  ...
  /*
   * [TEST] SPDU 내 생성시각이 인증서 유효기간 시작시점보다 과거여도, "성공"이 반환되는 것을 확인한다.
  ...
  /*
   * [TEST] SPDU 내 생성시각이 인증서 유효기간 종료시점보다 미래여도, "성공"이 반환되는 것을 확인한다.
