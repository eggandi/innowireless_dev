/** 
  * @file 
  * @brief 서명 SPDU의 만기에 대한 relevance 체크 기능 단위테스트
  * @date 2022-01-06
  * @author gyun
  */
...
/**
 * @brief Relevance check 시, SPDU가 만기되었을 때의 동작을 확인한다.
 */
TEST(PROCESS_SIGNED_SPDU_RELEVANCE_CHECK_EXPIRY_TIME, CHECK_EXPIRY_TIME)
{
  ...
  */
  ...
  profile.rx.relevance_check.exp_time = true; // SPDU 만기 조건을 체크하도록 한다.
  profile.rx.relevance_check.exp_time_src = kDot2RelevanceTimeSource_SecurityHeader; // Header에 있는 만기시각을 사용하도록 한다.
  ...
  /*
   * [TEST] SPDU 수신시각이 SPDU 만기시각보다 과거이면, "성공"이 반환되는 것을 확인한다.
   */
  {
    ...
    struct Dot2SPDUProcessParams params = {g_sample_max_header_signed_data_exp_time - 1ULL, 135, 374063230L, 1271023340L};
  ...
  /*
   * [TEST] SPDU 수신시각이 SPDU 만기시각과 같으면, "성공"이 반환되는 것을 확인한다.
   */
  {
    struct V2XPacketParseData *parsed = V2X_AllocateDSRCPacketParseData(nullptr, 0, nullptr);
    ASSERT_TRUE(parsed != nullptr);
    ...
    uint8_t *payload = g_sample_signed_data_payload;
    size_t payload_size = g_sample_signed_data_payload_size;
    struct Dot2SPDUProcessParams params = {g_sample_max_header_signed_data_exp_time, 135, 374063230L, 1271023340L};
    ASSERT_EQ(Dot2_ProcessSPDU(spdu, spdu_size, &params, parsed), kDot2Result_Success);
  ...
  /*
   * [TEST] SPDU 수신시각이 SPDU 만기시각보다 미래이면, "실패"가 반환되는 것을 확인한다.
   */
  {
    struct V2XPacketParseData *parsed = V2X_AllocateDSRCPacketParseData(nullptr, 0, nullptr);
    ASSERT_TRUE(parsed != nullptr);
    ...
    ASSERT_EQ(g_callbacks.entry[2].result, -kDot2Result_SPDURelevance_Expiry);
  }

  /*
   * [TEST] SPDU 내에 만기시각 정보가 없으면, 무조건 "성공"이 반환되는 것을 확인한다.
   */
  {
    struct V2XPacketParseData *parsed = V2X_AllocateDSRCPacketParseData(nullptr, 0, nullptr);
    ASSERT_TRUE(parsed != nullptr);
...
/**
 * @brief SPDU 만기에 대한 Relevance check 조건 미 설정 시, SPDU가 만기되었을 때의 동작을 확인한다.
 */
TEST(PROCESS_SIGNED_SPDU_RELEVANCE_CHECK_EXPIRY_TIME, NOT_CHECK_EXPIRY_TIME)
{
  ASSERT_EQ(Dot2_Init(kDot2LogLevel_Err, kDot2SigningParamsPrecomputeInterval_Default, "/dev/urandom", kDot2LeapSeconds_Default), kDot2Result_Success);
  Dot2_RegisterProcessSPDUCallback(Dot2Test_ProcessSPDUCallback);
  Dot2Test_InitProcessSPDUCallbackList();
  ...
  */
  ...
  profile.rx.relevance_check.gen_time_in_past = false;
  profile.rx.relevance_check.gen_time_in_future = false;
  profile.rx.relevance_check.exp_time = false; // SPDU 만기 조건을 체크하지 않도록 한다.
  profile.rx.relevance_check.gen_location_distance = false;
  profile.rx.relevance_check.cert_expiry = false;
  profile.rx.consistency_check.gen_location = false;
  ASSERT_EQ(Dot2_AddSecProfile(&profile), kDot2Result_Success);
  ...
  /*
   * [TEST] SPDU 수신시각이 SPDU 만기시각보다 미래여도(SPDU가 만기되었어도), "성공"이 반환되는 것을 확인한다.
   */
  {
    struct V2XPacketParseData *parsed = V2X_AllocateDSRCPacketParseData(nullptr, 0, nullptr);
    ASSERT_TRUE(parsed != nullptr);
...
/**
 * @brief 서명 SPDU를 검증하지 않도록 설정 시, SPDU가 만기되었을 때의 동작을 확인한다.
 */
TEST(PROCESS_SIGNED_SPDU_RELEVANCE_CHECK_EXPIRY_TIME, NOT_VERIFY)
{
  ASSERT_EQ(Dot2_Init(kDot2LogLevel_Err, kDot2SigningParamsPrecomputeInterval_Default, "/dev/urandom", kDot2LeapSeconds_Default), kDot2Result_Success);
  Dot2_RegisterProcessSPDUCallback(Dot2Test_ProcessSPDUCallback);
  Dot2Test_InitProcessSPDUCallbackList();
