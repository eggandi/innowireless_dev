/** 
  * @file 
  * @brief 서명 SPDU의 future generation에 관한 relevance 체크 기능 단위테스트
  * @date 2022-01-06
  * @author gyun
  */
...
/**
 * @brief 서명 SPDU의 future generation에 대한 relevance check 시, SPDU의 생성시각이 너무 미래일 경우의 동작을 확인한다.
 */
TEST(PROCESS_SIGNED_SPDU_RELEVANCE_CHECK_FUTURE_GENERATION, CHECK_FUTURE_GENERATION)
{
  ...
  */
  ...
  profile.rx.relevance_check.gen_time_in_future = true; // SPDU future generation 조건을 체크하도록 한다.
  profile.rx.relevance_check.acceptable_future_data_period = 100000ULL; // SPDU 생성시각이 수신시점에 비해 100msec보다(초과) 미래이면 필터링.
  ...
  /*
   * [TEST] SPDU 수신시각이 {생성시각 - 유효범위}보다 작으면(SPDU 생성시각이 너무 미래이면), "실패"가 반환되는 것을 확인한다.
   */
  {
    ...
    ASSERT_EQ(g_callbacks.entry[0].result, -kDot2Result_SPDURelevance_Future);
  }

  /*
   * [TEST] SPDU 수신시각이 {생성시각 - 유효범위}와 같으면(SPDU 생성시각이 허용될만한 미래이면), "성공"이 반환되는 것을 확인한다.
   */
  {
    struct V2XPacketParseData *parsed = V2X_AllocateDSRCPacketParseData(nullptr, 0, nullptr);
    ASSERT_TRUE(parsed != nullptr);
  ...
  /*
   * [TEST] SPDU 수신시각이 {생성시각 - 유효범위}보다 크면(SPDU 생성시각이 허용될만한 미래이면), "성공"이 반환되는 것을 확인한다.
   */
  {
    struct V2XPacketParseData *parsed = V2X_AllocateDSRCPacketParseData(nullptr, 0, nullptr);
    ASSERT_TRUE(parsed != nullptr);
...
/**
 * @brief 서명 SPDU의 future generation에 대한 relevance check 미수행 시, SPDU의 생성시각이 너무 미래일 경우의 동작을 확인한다.
 */
TEST(PROCESS_SIGNED_SPDU_RELEVANCE_CHECK_FUTURE_GENERATION, NOT_CHECK_FUTURE_GENERATION)
{
  ASSERT_EQ(Dot2_Init(kDot2LogLevel_Err, kDot2SigningParamsPrecomputeInterval_Default, "/dev/urandom", kDot2LeapSeconds_Default), kDot2Result_Success);
  Dot2_RegisterProcessSPDUCallback(Dot2Test_ProcessSPDUCallback);
  Dot2Test_InitProcessSPDUCallbackList();
  ...
  */
  ...
  profile.rx.relevance_check.replay = false;
  profile.rx.relevance_check.gen_time_in_past = false;
  profile.rx.relevance_check.gen_time_in_future = false; // SPDU future generation 조건을 체크하지 않도록 한다.
  profile.rx.relevance_check.exp_time = false;
  profile.rx.relevance_check.gen_location_distance = false;
  profile.rx.relevance_check.cert_expiry = false;
  profile.rx.consistency_check.gen_location = false;
  ...
  /*
   * [TEST] Security profile이 future generation check를 수행하지 않도록 설정되어 있는 경우,
   * 생성시각이 매우 미래여도 "성공"이 반환되는 것을 확인한다.
   */
  {
    struct V2XPacketParseData *parsed = V2X_AllocateDSRCPacketParseData(nullptr, 0, nullptr);
    ASSERT_TRUE(parsed != nullptr);
...
/**
 * @brief 서명 SPDU를 검증하지 않도록 설정 시, SPDU의 생성시각이 너무 미래일 경우의 동작을 확인한다.
 */
TEST(PROCESS_SIGNED_SPDU_RELEVANCE_CHECK_FUTURE_GENERATION, NOT_VERIFY)
{
  ASSERT_EQ(Dot2_Init(kDot2LogLevel_Err, kDot2SigningParamsPrecomputeInterval_Default, "/dev/urandom", kDot2LeapSeconds_Default), kDot2Result_Success);
  Dot2_RegisterProcessSPDUCallback(Dot2Test_ProcessSPDUCallback);
  Dot2Test_InitProcessSPDUCallbackList();
  ...
  */
  ...
  /*
   * [TEST] 생성시각이 매우 미래여도 "성공"이 반환되는 것을 확인한다.
   */
  {
    struct V2XPacketParseData *parsed = V2X_AllocateDSRCPacketParseData(nullptr, 0, nullptr);
    ASSERT_TRUE(parsed != nullptr);
