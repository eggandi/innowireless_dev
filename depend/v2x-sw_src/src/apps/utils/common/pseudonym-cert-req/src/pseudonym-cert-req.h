/** 
  * @file 
  * @brief 익명인증서 발급/다운로드 유틸리티 관련 정의
  * @date 2022-07-28 
  * @author gyun 
  */

#ifndef V2X_SW_PSEUDONYM_CERT_REQ_H
#define V2X_SW_PSEUDONYM_CERT_REQ_H
 ...
 * @brief 유틸리티 동작 유형
 */
enum ePseudonymCertReqOperationType
{
  kPseudonymCertReqOperationType_Req, ///< 익명인증서 발급 요청 동작
  kPseudonymCertReqOperationType_Down, ///< 익명인증서 다운로드 동작
  kPseudonymCertReqOperationType_Info, ///< 익명인증서 다운로드일정정보 다운로드 동작
};
typedef unsigned int PseudonymCertReqOperationType; ///< @ref ePseudonymCertReqOperationType


/**
 * @brief 유틸리티 설정 정보
 */
struct PseudonymCertReqCFG
{
  PseudonymCertReqOperationType op; ///< 동작 유형
  ...
  struct {
    ...
    char cmhf_dir[MAXLINE + 1]; ///< 익명인증서 CMHF가 저장될 디렉토리
    char cert_dir[MAXLINE + 1]; ///< 익명인증서 바이트열이 저장될 디렉토리
    char tmp_zip_file[MAXLINE + 1]; ///< 서버에서 다운로드한 ZIP 파일이 임시로 저장될 파일 경로
    Dot2IPeriod i_period; ///< 발급받고자 하는 인증서에 대한 i-period
 ...
 * 전역 변수
 */
extern struct PseudonymCertReqCFG g_cfg;
...
extern const char *g_default_pseudonym_cert_cmhf_dir;
extern const char *g_default_pseudonym_cert_dir;
...
extern const char *g_default_pseudonym_cert_provisioning_req_h8_file;
extern const char *g_default_pseudonym_cert_provisioning_req_file;
extern const char *g_default_pseudonym_cert_provisioning_ack_file;
extern const char *g_default_cert_download_time_file;
extern const char *g_default_tmp_zip_file_dir;
extern Dot2IPeriod g_default_i_period;
extern const char *g_default_pseudonym_cert_provisiong_req_url;
extern const char *g_default_pseudonym_cert_download_req_url;
extern const char *g_default_pseudonym_cert_download_info_req_url;
...
int PSEUDONYM_CERT_REQ_RequestPseudonymCertProvisioning(void);
int PSEUDONYM_CERT_REQ_DownloadPseudonymCert(void);
int PSEUDONYM_CERT_REQ_DownloadPseudonymCertDownloadInfo(void);
int PSEUDONYM_CERT_REQ_LoadSCCCertFiles(void);
int PSEUDONYM_CERT_REQ_LoadEnrollmentCMHFFile(void);

#endif //V2X_SW_PSEUDONYM_CERT_REQ_H
