/** 
  * @file 
  * @brief 익명인증서 발급 유틸리티 입력 파라미터 처리 기능 구현
  * @date 2022-07-28 
  * @author gyun 
  */
 ...
 * @param[in] op 동작 유형
 */
static void PSEUDONYM_CERT_REQ_SetDefaultInputParameters(PseudonymCertReqOperationType op)
{
  ...
  snprintf(g_cfg.req_h8_file, MAXLINE, "%s", g_default_pseudonym_cert_provisioning_req_h8_file);

  if (op == kPseudonymCertReqOperationType_Req) {
    snprintf(g_cfg.req.provisiong_req_url, MAXLINE, "%s", g_default_pseudonym_cert_provisiong_req_url);
    snprintf(g_cfg.req.req_file, MAXLINE, "%s", g_default_pseudonym_cert_provisioning_req_file);
    snprintf(g_cfg.req.ack_file, MAXLINE, "%s", g_default_pseudonym_cert_provisioning_ack_file);
    snprintf(g_cfg.req.cert_dl_time_file, MAXLINE, "%s", g_default_cert_download_time_file);
  } else if (op == kPseudonymCertReqOperationType_Down) {
    snprintf(g_cfg.down.download_req_url, MAXLINE, "%s", g_default_pseudonym_cert_download_req_url);
    g_cfg.down.i_period = g_default_i_period;
    snprintf(g_cfg.down.cmhf_dir, MAXLINE, "%s", g_default_pseudonym_cert_cmhf_dir);
    snprintf(g_cfg.down.cert_dir, MAXLINE, "%s", g_default_pseudonym_cert_dir);
    snprintf(g_cfg.down.tmp_zip_file, MAXLINE, "%s", g_default_tmp_zip_file_dir);
  } else if (op == kPseudonymCertReqOperationType_Info) {
    snprintf(g_cfg.info.download_info_req_url, MAXLINE, "%s", g_default_pseudonym_cert_download_info_req_url);
  }
}
 ...
 * @retval -1: 실패
 */
static int PSEUDONYM_CERT_REQ_ProcessParsedOption(int option)
{
  int ret = 0;
  switch (option) {
    case 0: { // "rca"
    ...
    case 19: { // i
      if (g_cfg.op == kPseudonymCertReqOperationType_Down) {
        g_cfg.down.i_period = (Dot2IPeriod)strtoul(optarg, 0, 16);
      }
      break;
    }
    case 20: { // libdbg
 ...
 * @retval -1: 실패
 */
int PSEUDONYM_CERT_REQ_ParsingInputParameters(int argc, char *argv[])
{
  int c, option_idx = 0;
  ...
  if (!memcmp(argv[1], "req", 3)) {
    g_cfg.op = kPseudonymCertReqOperationType_Req;
  } else if (!memcmp(argv[1], "down", 4)) {
    g_cfg.op = kPseudonymCertReqOperationType_Down;
  } else if (!memcmp(argv[1], "info", 4)) {
    g_cfg.op = kPseudonymCertReqOperationType_Info;
  } else {
    printf("Invalid operation - %s\n", argv[1]);
    return -1;
  }
  PSEUDONYM_CERT_REQ_SetDefaultInputParameters(g_cfg.op);
  ...
  while(1) {
    ...
    // 파싱된 옵션 처리 -> 저장
    int ret = PSEUDONYM_CERT_REQ_ProcessParsedOption(c);
    if (ret < 0) {
      return ret;
    }
  }
