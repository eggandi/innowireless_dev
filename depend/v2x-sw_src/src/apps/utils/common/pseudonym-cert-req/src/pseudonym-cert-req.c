/** 
  * @file 
  * @brief 익명인증서 발급요청/다운로드 유틸리티 메인 파일
  * @date 2022-07-28 
  * @author gyun 
  */
...
const char *g_default_pseudonym_cert_cmhf_dir = "./";
const char *g_default_pseudonym_cert_dir = "./";
...
const char *g_default_pseudonym_cert_provisioning_req_h8_file = "SecuredPseudonymCertProvisioningRequest.oer.H8";
const char *g_default_pseudonym_cert_provisioning_req_file = "SecuredPseudonymCertProvisioningRequest.oer";
const char *g_default_pseudonym_cert_provisioning_ack_file = "SignedPseudonymCertProvisioningAck.oer";
const char *g_default_cert_download_time_file = "CertDownTime.txt";
const char *g_default_tmp_zip_file_dir = "./down.zip";
Dot2IPeriod g_default_i_period = kDot2IPeriod_Current;
const char *g_default_pseudonym_cert_provisiong_req_url = "https://ra.scms.co.kr:8892/provision-pseudonym-certificate-batch";
const char *g_default_pseudonym_cert_download_req_url = "https://ra.scms.co.kr:8892/download/batch";
const char *g_default_pseudonym_cert_download_info_req_url = "https://ra.scms.co.kr:8892/download/info";

struct PseudonymCertReqCFG g_cfg; ///< 유틸리티 설정정보
 ...
 */
static void PSEUDONYM_CERT_REQ_Usage(const char *app_filename)
{
  printf("\n\n Description: Pseudonym cert request utility using v2x-sw libraries\n");
  printf(" Version: %s\n", _VERSION_);
  printf(" Author: gyun\n");
  printf(" Email: junghg@keti.re.kr\n");

  printf("\n Usage: %s req|down|info [OPTIONS]\n\n", app_filename);
  printf("          req: Request pseudonym cert provisioning\n");
  printf("          down: Download pseudonym certs (1 set(=20 certs) for 1 weeks)\n");
  printf("          info: Download pseudonym cert donwload info (possible time to download next duration certs)\n");
  ...
  printf("  --url <URL>              [INPUT]  Set URL for pseudonym cert provisiong request. If not specified, set to \"%s\"\n", g_default_pseudonym_cert_provisiong_req_url);
  ...
  printf("  --h8 <file path>         [OUTPUT] Set H8(SecuredPseudonymCertProvisioningRequest) file export path. If not specified, set to \"%s\"\n", g_default_pseudonym_cert_provisioning_req_h8_file);
  printf("  --req <file path>        [OUTPUT] Set OER-encoded SecuredPseudonymCertProvisioningRequest file export path. If not specified, set to \"%s\"\n", g_default_pseudonym_cert_provisioning_req_file);
  printf("  --ack <file path>        [OUTPUT] Set OER-encoded SignedPseudonymCertProvisioningAck file export path. If not specified, set to \"%s\"\n", g_default_pseudonym_cert_provisioning_ack_file);
  ...
  printf("  --h8 <file path>         [INPUT]  Set H8(SecuredPseudonymCertProvisioningRequest) file path. If not specified, set to \"%s\"\n", g_default_pseudonym_cert_provisioning_req_h8_file);
  printf("  --url <URL>              [INPUT]  Set URL for pseudonym cert download request. If not specified, set to \"%s\"\n", g_default_pseudonym_cert_download_req_url);
  printf("  --i <hex value>          [INPUT]  Set i-period. If not specified, set to 0x%X\n", g_default_i_period);
  printf("  --cmhf <dir path>        [OUTPUT] Set pseudonym cert CMHF file export dir path. If not specified, set to \"%s\"\n", g_default_pseudonym_cert_cmhf_dir);
  printf("  --cert <dir path>        [OUTPUT] Set OER-encoded pseudonym cert/privkey/recon_priv file export dir path. If not specified, set to \"%s\"\n", g_default_pseudonym_cert_dir);
  ...
  printf("  --enroll <dir path>      [INPUT]  Set enrollment cert CMHF dir path, If not specified, set to \"%s\"\n", g_default_enroll_cert_cmhf_dir);
  printf("  --h8 <file path>         [INPUT]  Set H8(SecuredPseudonymCertProvisioningRequest) file path. If not specified, set to \"%s\"\n", g_default_pseudonym_cert_provisioning_req_h8_file);
  printf("  --url <URL>              [INPUT]  Set URL for pseudonym cert download info request. If not specified, set to \"%s\"\n", g_default_pseudonym_cert_download_info_req_url);
  printf("  --libdbg <level>                  Set v2x libraries debug message print level. If not specified, set to %u\n", DEFAULT_LIB_DBG);
  printf("                                        0: nothing, 1: err, 2: init, 3: event, 4: message hexdump\n");
...
int main(int argc, char *argv[])
{
   ...
   */
  if (argc < 2) {
    PSEUDONYM_CERT_REQ_Usage(argv[0]);
    return 0;
  }

  printf("Running pseudonym cert (for OBU) request utility\n");
  memset(&g_cfg, 0, sizeof(g_cfg));

  /*
   * 입력 파라미터를 파싱하여 저장한다.
   */
  int ret = PSEUDONYM_CERT_REQ_ParsingInputParameters(argc, argv);
  if (ret < 0) {
    return -1;
  }
   ...
   * SCC 인증서들을 로딩한다.
   */
  ret = PSEUDONYM_CERT_REQ_LoadSCCCertFiles();
  if (ret < 0) {
    return -1;
  }
   ...
   * 등록인증서 CMHF를 로딩한다.
   */
  ret = PSEUDONYM_CERT_REQ_LoadEnrollmentCMHFFile();
  if (ret < 0) {
    return -1;
  }
   ...
   * LCM 설정을 수행한다.
   */
  ret = PSEUDONYM_CERT_REQ_ConfigLCM();
  if (ret < 0) {
    return -1;
  }
   ...
   * 동작 유형에 따른 동작을 수행한다.
   */
  if (g_cfg.op == kPseudonymCertReqOperationType_Req) {
    ret = PSEUDONYM_CERT_REQ_RequestPseudonymCertProvisioning();
  } else if (g_cfg.op == kPseudonymCertReqOperationType_Down) {
    ret = PSEUDONYM_CERT_REQ_DownloadPseudonymCert();
  } else if (g_cfg.op == kPseudonymCertReqOperationType_Info) {
    ret = PSEUDONYM_CERT_REQ_DownloadPseudonymCertDownloadInfo();
  }
 ...
 */
void PSEUDONYM_CERT_REQ_PrintCFG(void)
{
  if (g_cfg.op == kPseudonymCertReqOperationType_Req) {
    ...
    printf("  [OUTPUT] H8(SecuredPseudonymCertProvisioningRequest) file export path: %s\n", g_cfg.req_h8_file);
    printf("  [OUTPUT] SecuredPseudonymCertProvisioningRequest file export path: %s\n", g_cfg.req.req_file);
    printf("  [OUTPUT] SignedPseudonymCertProvisioningAck file export path: %s\n", g_cfg.req.ack_file);
    ...
    printf("  [INPUT]  H8(SecuredPseudonymCertProvisioningRequest) file path: %s\n", g_cfg.req_h8_file);
    printf("  [INPUT]  Download request URL: %s\n", g_cfg.down.download_req_url);
    printf("  [INPUT]  i-period: 0x%X\n", g_cfg.down.i_period);
    printf("  [OUTPUT] Pseudonym cert CMHF file export directory: %s\n", g_cfg.down.cmhf_dir);
    printf("  [OUTPUT] Pseudonym cert file export directory: %s\n", g_cfg.down.cert_dir);
 ...
 * @retval -1: 실패
 */
int PSEUDONYM_CERT_REQ_ConfigLCM(void)
{
  /*
  ...
  /*
   * 익명인증서 발급요청 URL을 설정한다.
   */
  if (g_cfg.op == kPseudonymCertReqOperationType_Req) {
    ret = Dot2_ConfigLCM(kDot2LCMConfigType_PseudonymCertProvisioningReqURL, g_cfg.req.provisiong_req_url);
    if (ret < 0) {
      return -1;
    }
  }
   ...
   */
  if (g_cfg.op == kPseudonymCertReqOperationType_Down) {
 ...
 */
void PSEUDONYM_CERT_REQ_PrintOcts(const char *desc, const uint8_t *octs, size_t len)
{
