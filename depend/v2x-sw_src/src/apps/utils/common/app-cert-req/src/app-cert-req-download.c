...
int APP_CERT_REQ_DownloadAppCert(void)
{
  ...
  int ret = APP_CERT_REQ_ImportFile(g_cfg.v_file, verify_priv_key.octs, DOT2_EC_256_KEY_LEN, DOT2_EC_256_KEY_LEN);
  if (ret < 0) {
  ...
  ret = APP_CERT_REQ_ImportFile(g_cfg.e_file, cert_enc_priv_key.octs, DOT2_EC_256_KEY_LEN, DOT2_EC_256_KEY_LEN);
  if (ret < 0) {
    printf("Fail to import %s\n", g_cfg.e_file);
    return -1;
  }
  // 인증서발급요청문 H8
  uint8_t req_h8[8];
  ret = APP_CERT_REQ_ImportFile(g_cfg.req_h8_file, req_h8, 8, 8);
  if (ret < 0) {
    printf("Fail to import %s\n", g_cfg.req_h8_file);
    return -1;
  }
  ...
  params.return_options = true; // 옵션정보들도 함께 리턴하도록 설정
  ...
  if (res.ret < 0) {
    printf("Fail to Dot2_DownloadAppCert() : %d\n", res.ret);
    return -1;
  }
  ...
  APP_CERT_REQ_PrintOcts("CMHF", res.common.cmhf, res.common.cmhf_size);
  if (params.return_options == true) {
    printf("Directory name: %s\n", res.options.dir_name);
    printf("Cert Filename: %s\n", res.options.cert_filename);
    APP_CERT_REQ_PrintOcts("Cert", res.options.cert.octs, res.options.cert.size);
    printf("PrivKey name: %s\n", res.options.priv_key_filename);
    APP_CERT_REQ_PrintOcts("PrivKey", res.options.priv_key.octs, sizeof(res.options.priv_key.octs));
    printf("ReconPriv name: %s\n", res.options.recon_priv_filename);
    APP_CERT_REQ_PrintOcts("ReconPriv", res.options.recon_priv.octs, sizeof(res.options.recon_priv.octs));
  }
   ...
   */
  // CMHF 파일
  ret = APP_CERT_REQ_ExportDirFile(g_cfg.down.cert_dir, res.common.cmhf_name, res.common.cmhf, res.common.cmhf_size);
  if (ret < 0) {
    goto out;
  }
  // 옵션 정보
  if (params.return_options == true) {
    ...
    // 인증서 파일 저장
    ret = APP_CERT_REQ_ExportDirFile(g_cfg.down.cert_dir,
                                     res.options.cert_filename,
