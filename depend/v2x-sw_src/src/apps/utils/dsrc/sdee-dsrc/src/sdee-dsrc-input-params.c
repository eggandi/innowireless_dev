/** 
 * @file
 * @brief sdee-dsrc 샘플 어플리케이션 실행 시 함께 입력되는 파라미터들을 파싱/처리하는 기능 구현 파일
 * @date 2020-05-26
 * @author gyun
 */
 ...
 * @retval -1: 실패
 */
static int SDEE_DSRC_ProcessParsedOption(int option)
{
  switch (option) {
    case 0: {
      if (*optarg == 'u') {
...
int SDEE_DSRC_ParsingInputParameters(int argc, char *argv[])
{
  ...
  struct option options[] = {
    ...
    {"len", required_argument, 0, 2},
    {"interval", required_argument, 0, 3},
    {"lat", required_argument, 0, 4},
    {"lon", required_argument, 0, 5},
    {"cmhf", required_argument, 0, 6},
    {"rca", required_argument, 0, 7},
    {"ica", required_argument, 0, 8},
    {"pca", required_argument, 0, 9},
...
#ifdef _X64_
    printf("RX operation in x64 system is not supported\n");
    return -1;
#endif
  ...
  while(1)
  {
    ...
    // 파싱된 옵션 처리
    ret = SDEE_DSRC_ProcessParsedOption(c);
    if (ret < 0) {
      return ret;
    }
  }
   ...
   * 파싱된 파라미터 내용 출력
   */
  printf("  op: %u(0:rx,1:trx,2:loopback), PSID: %u, RCA: %s, ICA: %s, PCA: %s, dbg:%u, lib dbg: %u\n",
         g_mib.op, g_mib.psid, g_mib.rca_cert_file_path, g_mib.ica_cert_file_path,
