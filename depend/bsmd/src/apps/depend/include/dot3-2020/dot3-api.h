...
/**
 * @brief LCI(LTE-V2X Channel Info)를 추가/업데이트한다(상세 내용 API 매뉴얼 참조).
 * @param[in] lci 추가할 LCI 정보가 담긴 정보 구조체 포인터
 * @retval 0 이상: 테이블에 등록되어 있는 LCI의 개수
 * @retval 음수(-Dot3ResultCode): 실패
 */
int Dot3_AddLCI(const struct Dot3LCI *lci);


/**
 * @brief 모든 LCI(LTE-V2X Channel Info)들을 삭제한다(상세 내용 API 매뉴얼 참조).
 */
void Dot3_DeleteAllLCIs(void);


/**
 * @brief 특정 EARFCN(채널번호)을 갖는 LCI(LTE-V2X Channel Info)를 구한다(상세 내용 API 매뉴얼 참조).
 * @param[in] eafcn 구하고자 하는 LCI의 EARFCN(채널번호)
 * @param[out] lci LCI 정보가 반환될 변수의 포인터
 * @retval 0: 성공
 * @retval 음수(-Dot3ResultCode): 실패
 */
int Dot3_GetLCIWithEARFCN(Dot3LTEv2xAfrcnEutraR9 earfcn, struct Dot3LCI *lci);


/**
 * @brief 등록되어 있는 LCI(LTE-V2X Channel Info)의 개수를 확인한다(상세 내용 API 매뉴얼 참조).
 * @return 등록되어 있는 LCI의 개수
 */
Dot3LCINum Dot3_GetLCINum(void);
