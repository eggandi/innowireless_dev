...
/// 로그 출력 매크로
#define Log(l, f, a...) do { if (g_mib.dbg >= l) { WSM_LTEV2X_PrintLog(__FUNCTION__, f, ## a); } } while(0)
/// 에러레벨 로그 출력 매크로
#define Err(f, a ...) do { if (g_mib.dbg >= kDbgMsgLevel_err) { WSM_LTEV2X_PrintLog(__FUNCTION__,f,## a); } } while(0)
#else
...
struct MIB
{
  ...
  LTEV2XHALTxFlowType tx_flow_type; ///< 송신 유형
...
void WSM_LTEV2X_PrintLog(const char *func, const char *format, ...);
int WSM_LTEV2X_ParsingInputParameters(int argc, char *argv[]);
void WSM_LTEV2X_ProcessRxMSDUCallback(const uint8_t *msdu, LTEV2XHALMSDUSize msdu_size, struct LTEV2XHALMSDURxParams rx_params);
int WSM_LTEV2X_InitTxOperation(unsigned int interval);

/* wsm-asn.c */
int WSM_LTEV2X_FillVarLengthNumber(int from, dot3VarLengthNumber *to);
int WSM_LTEV2X_ParseVarLengthNumber(const dot3VarLengthNumber *var_len_num);
