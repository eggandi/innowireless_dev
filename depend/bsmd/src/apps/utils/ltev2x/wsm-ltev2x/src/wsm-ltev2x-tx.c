 ...
 */
static void WSM_LTEV2X_TransmitWSM(const uint8_t *wsdu, size_t wsdu_size)
{
  int ret;
  ...
  *datarate = (int) g_mib.tx_datarate;
  ...
  ret = WSM_LTEV2X_FillVarLengthNumber(g_mib.psid, &wsm->transport.u.bcMode.destAddress);
  if (ret < 0) {
    Log(kDbgMsgLevel_Err, "Fail to fill destAddress(VarLengthNumber) - WSM_LTEV2X_FillVarLengthNumber() failed: %d\n", ret);
    goto clear;
  }
 ...
 */
static void WSM_LTEV2X_ReleaseTxTimer()
{
 ...
 */
static void WSM_LTEV2X_TxTimerThread(union sigval arg)
{
  (void)arg;

  if (g_mib.tx_running == false) {
    WSM_LTEV2X_ReleaseTxTimer();
  }
  ...
  // WSM을 생성하여 전송한다.
  WSM_LTEV2X_TransmitWSM(g_payload, g_mib.tx_wsm_body_len);
}
 ...
 * @retval -1: 실패
 */
static int WSM_LTEV2X_InitTxTimer(unsigned int interval)
{
  int ret;
  ...
  /*
   * 송신타이머 만기 시 송신타이머쓰레드(WSM_LTEV2X_TxTimerThread)가 생성되도록 설정한다.
   */
  se.sigev_notify = SIGEV_THREAD;
  se.sigev_value.sival_ptr = &g_tx_timer;
  se.sigev_notify_function = WSM_LTEV2X_TxTimerThread;
 ...
 * @retval -1: 실패
 */
int WSM_LTEV2X_InitTxOperation(unsigned int interval)
{
  Log(kDbgMsgLevel_Init, "Initialize WSM tx operation\n");
  int ret;

  // 송신 플로우를 등록한다.
  if (g_mib.tx_flow_type == kLTEV2XHALTxFlowType_SPS) {
  ...
  // 송신 타이머를 생성한다.
  ret = WSM_LTEV2X_InitTxTimer(interval);
  if (ret < 0) {
    return -1;
  }
