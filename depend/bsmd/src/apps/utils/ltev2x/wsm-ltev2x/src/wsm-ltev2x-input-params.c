 ...
 * @retval -1: 실패
 */
static int WSM_LTEV2X_ProcessParsedOption(int option)
{
  switch (option) {
    case 1: {
...
int WSM_LTEV2X_ParsingInputParameters(int argc, char *argv[])
{
  ...
  while(1) {
     ...
     * 파싱된 옵션 처리
     */
    int ret = WSM_LTEV2X_ProcessParsedOption(c);
    if (ret < 0) {
      return ret;
    }
  }
   ...
   */
  printf("  dev: %s\n", g_mib.dev_name);
  printf("  op: %u(0:rx,1:trx), tx_flow_type: %u(0:event, 1:SPS), PSID: %u, dbg: %u, lib dbg: %u\n",
         g_mib.op, g_mib.tx_flow_type, g_mib.psid, g_mib.dbg, g_mib.lib_dbg);
