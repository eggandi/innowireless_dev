...
static uint8_t g_msg_cnt = 0;
static uint8_t g_temporary_id[TEMPORARY_ID_LEN] = { 0x00, 0x01, 0x02, 0x03 };
...
static int g_radius_of_curve = 10;
static int g_confidence = 11;

static float RELAY_INNO_BSM_SecMark();
...
API uint8_t *REPLAY_INNO_J2736_Construct_BSM(size_t *bsm_size)
{
  ...
  // BSM을 인코딩한다.
  *bsm_size = (size_t)asn1_uper_encode(&buf, asn1_type_v2xtestMessageFrame, frame);
  if (buf == NULL) {
    printf("Fail to encode BSM - asn1_uper_encode() failed\n");
    goto out;
  }
...
out:
  if (frame) 
	{ 
		asn1_free_value(asn1_type_v2xtestMessageFrame, frame); 
	}
  return buf;
}
/**
...
API int RELAY_INNO_J2735_Fill_BSM(struct j2735BasicSafetyMessage *bsm)
{
	...
	{
		RELAY_INNO_BSM_Gnss_Info_Ptr_Instral(NULL);
	}
...
API void RELAY_INNO_BSM_Gnss_Info_Ptr_Instrall(struct j2735BSMcoreData **core_ptr)
{
	...
	G_gnss_bsm_data->status = &core->status;
	...
	G_gnss_bsm_data->acceleration_set.lon = (int*)&core->accelSet.lon;
	G_gnss_bsm_data->acceleration_set.vert = (int*)&core->accelSet.vert;
	G_gnss_bsm_data->acceleration_set.yaw = (int*)&core->accelSet.yaw;
	G_gnss_bsm_data->pos_accuracy.semi_major = (unsigned int*)&core->accuracy.semi_major;
	G_gnss_bsm_data->pos_accuracy.semi_minor = (unsigned int*)&core->accuracy.semi_minor;
...
static int RELAY_INNO_BSM_Fill_CoreData(struct j2735BSMcoreData *core_ptr)
{
  ...
  {
    core->msgCnt = RELAY_INNO_INCREASE_BSM_MSG_CNT(g_msg_cnt);
